...
async function runTests() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`   Original: ${data.originalLength} chars`);
      console.log(`   Simplified: ${data.simplifiedLength} chars`);
      console.log(`   Simplified text: ${data.simplifiedText.substring(0, 100)}...`);
      console.log(`   Bullets: ${data.bullets ? data.bullets.length : 0} items`);
      console.log(`   Key points: ${data.keyPoints ? data.keyPoints.length : 0} items`);
      console.log(`   Flow steps: ${data.flow ? data.flow.length : 0} items\n`);
    } else {
      const error = await response.json();
      console.log(`❌ Server error (${response.status}): ${error.error}\n`);
    }
  } catch (error) {
    console.log(`❌ Request failed: ${error.message}\n`);
  }
